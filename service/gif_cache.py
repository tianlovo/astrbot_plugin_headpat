...
class GifCacheService:
    ...
    def __init__(self, plugin_name: str, config: dict = None):
        ...
        logger.info(
            f"[{plugin_name}] GIF缓存服务初始化完成，缓存目录: {self.cache_dir}"
        )
    ...
    def _generate_cache_key(
        self, user_id: str, avatar_hash: str | None = None
    ) -> str:
        """生成缓存键
    ...
    def get(self, user_id: str, avatar_hash: str | None = None) -> Path | None:
        """获取缓存的 GIF
    ...
    def set(
        self, user_id: str, gif_path: Path, avatar_hash: str | None = None
    ) -> Path:
        """存储 GIF 到缓存
    ...
    def delete(
        self, user_id: str | None = None, avatar_hash: str | None = None
    ) -> bool:
        """删除缓存
    ...
    def start_auto_cleanup(self):
        ...
        if self._cleanup_task is None or self._cleanup_task.done():
            ...
            logger.info(
                f"[{self.plugin_name}] 自动清理任务已启动，间隔: {self.cleanup_interval}秒"
            )
